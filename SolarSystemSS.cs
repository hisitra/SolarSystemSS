...
public class SolarSystemSS : MonoBehaviour {
    ...
    void Start() {
        ...
        gamePaused = true; //game is paused when it starts so that the sudden mouse doesn't jerk the view away
        ...
        private void CreateBodies() {
            ...
            foreach (var body in solarSystemData) {
                ...
                go.GetComponent<MeshRenderer>().sharedMaterial = new Material(Shader.Find("Nature/Terrain/Standard"));
    ...
    void FixedUpdate() {
        ...
        //camera moves only when game is not paused
        if (!gamePaused) {
